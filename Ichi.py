...
class IchiCard:
    ...
    def __repr__(self):
        return '<IchiCard object: {} {}>'.format(self.color, self.card_type)
...
class IchiPlayer:
    def __init__(self, cards, player_id=None):
        if len(cards) != 7:
            raise ValueError(
                'Invalid player: must be initalised with 7 IchiCards'
            )
        if not all(isinstance(card, IchiCard) for card in cards):
            raise ValueError(
                'Invalid player: cards must all be IchiCard objects'
            )
        self.hand = cards
    ...
    def __repr__(self):
        if self.player_id is not None:
            return '<IchiPlayer object: player {}>'.format(self.player_id)
        else:
            return '<IchiPlayer object>'
...
class IchiGame:
    def __init__(self, players, random=True):
        ...
        self.players = [
            IchiPlayer(self._deal_hand(), n) for n in range(players)
        ]
    ...
    def _create_deck(self, random):
        ...
        deck = [IchiCard(color, card_type) for color, card_type in all_cards]
        if random:
            shuffle(deck)
            return deck
        else:
